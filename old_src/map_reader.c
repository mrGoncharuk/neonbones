...
/*   Updated: 2019/05/23 17:27:47 by mhonchar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_get_map_info(int fd, t_win *win)
{
	char	*line;
	char	**map_data;
	int		i;
	...
	win->posX = ft_atoi(map_data[2]);
	win->posY = ft_atoi(map_data[3]);
	ft_free_2darr(map_data);
...
int			ft_get_map_data(t_win *win, t_list *f)
{
	int		i;
	int		j;
	char	**data;
...
int			ft_list_init(t_list **f, t_win *win, int fd)
{
	t_list	*next;
	int		rows;
...
//TODO: test ft_is_map_valid()
int		ft_is_map_valid(t_win *win)
{
	int		j;

	if (win->pos.y > win->map.height || win->pos.x > win->map.width ||
		win->pos.y < 0 || win->pos.x < 0)
		return (E_BAD_MAP);
	if (win->map.data[win->pos.y - 1][win->pos.x - 1] != 0)
		return (E_BAD_MAP);
	j = -1;
